...
{
  unsigned int ra, rd, ro;
  unsigned int tmout, tmcnt; //time-out

  //Send DMI command via JTAG interface
  sprintf(message,"jtagd_dmi_req 0x%02x 0x%08x 0x%01x\n", r.addr, r.data, r.op);
  if(log)
    printf("req : 0x%02x 0x%08x 0x%01x\n", r.addr, r.data, r.op);
  ...
  //Change JTAG DMI command to receive response
  sprintf(message, "jtagd_dmi_resp\n");

  //Add time-out for dead-lock
  tmout = 100;
  tmcnt = 0;
    ...
    else if (strstr(response,"okay"))
    {
      ...
      if( (ra == r.addr) && (ro == 0x00)) {
        ...
        if(tmcnt <= tmout) {
          ...
          printf("last req : 0x%02x 0x%08x 0x%01x\n", r.addr, r.data, r.op);
          printf("last resp: 0x%02x 0x%08x 0x%01x\n", ra, rd, ro);
          printf("Exceed JTAG response time-out.\n");
          exit(1);
        }
      }
...
void dtmxsdb_t::producer_thread()
{
  ...
  if(exit_code != 0){
    ...
    printf(" [%s]:", test_name); //Add test case name
    if(!log)
      printf("Fail\n");
    else
      printf("Fail, %ld\n", jd_cnt);
...
void dtmxsdb_t::start_host_thread()
{
  ...
  //Reset JTAG
  sprintf(message, "jtagd_dmi_init\n");
  if( send(sock , message , strlen(message) , 0) < 0)
  {
    puts("Send failed");
    exit(1);
  ...
  while(1)
  {
    if( recv(sock , response , 50 , 0) < 0)
    {
      ...
      jd_cnt = 0; //Initialize JTAG DMI count
      break; //exit while(1)
    }
  ...
  : htif_t(args)
{
  test_name = NULL;
  if(!args.empty()) {
    test_name = (char*)args[0].c_str();
    for(int j = 0; test_name[j] != '\0'; j += 1) {
      if((test_name[j] == '/') && (test_name[j+1] != '\0')) {
        test_name += (j + 1);
        j = 0;
      }
    }
